...
#[derive(Accounts)]
pub struct SwapPoolBaseOut<'info> {
...
pub fn swap_base_out(ctx: Context<SwapPoolBaseOut>, max_amount_in: u64, amount_out: u64) -> Result<()> {
    ...
    msg!("max_amount_in: {}", max_amount_in);
    msg!("amount_out: {}", amount_out);
    let cpi_context = CpiContext::new(ctx.accounts.amm_program.to_account_info(), cpi_accounts);
    raydium_amm_cpi::instructions::swap_base_out(cpi_context, max_amount_in, amount_out)?;
